...
#And lose the
#For who would
#The pangs of d... Goodle helps
word_for_xoring = "For who would "
length_of_list = len(list_after_xor_between_each)
print(length_of_list)
strings_after_xoring = []
for i in list_after_xor_between_each:
    xored_full_length = ""
    for k in range(len(word_for_xoring)):
        xored = chr(operator.xor(ord(word_for_xoring[k]), ord(i[k])))
        listToStr = ''.join([str(elem) for elem in xored])
        xored_full_length += (listToStr)
    print(xored_full_length)
