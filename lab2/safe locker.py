...
for i in poem_list_hex:
    poem_list_text.append(binascii.unhexlify(i))
list_after_xor_between_each = []
for i in range(len(poem_list_text)):
    for k in range(i, len(poem_list_text)):
        list_int_to_chr = []
        new = [a ^ b for a, b in zip(poem_list_text[i], poem_list_text[k])]
        #print(i, "--", k)
        for l in new:
            #print(chr(l))
            list_int_to_chr.append(chr(l))
        print(len(list_int_to_chr))
        list_after_xor_between_each.append(list_int_to_chr)

word_for_xoring = "The"
length_of_list = len(list_after_xor_between_each)
print(length_of_list)
strings_after_xoring = []
for i in range(len(list_after_xor_between_each)):
    three_symbols = []
    three_symbols_after_xoring = []
    for o in range(0, len(list_after_xor_between_each[i]) - 3, 3):
        for l in range(o, o + 3):
            three_symbols.append(list_after_xor_between_each[i][l])
        for k in three_symbols:
            new = [(ord(a) ^ ord(b)) for a,b in zip(word_for_xoring, k)]
        for m in new:
            three_symbols_after_xoring.append(chr(m))
    strings_after_xoring.append(three_symbols_after_xoring)

#for i in strings_after_xoring:
    #print(i)
#for i in list_after_xor_between_each:
    #print(i)
